...
void forces(const int, const double[], double[], const double, const double, double**, const int);
...
int main(){
  ...
  double **tmpf = malloc(nb_threads * sizeof(double));
  for(int r = 0; r < nb_threads; r++) {
    tmpf[r] = calloc(3 * npart, sizeof(double));
  }
  ...
  */
  ...
  #pragma omp parallel private(move,vel,ekin,sc)
  {
    for (move = 1; move <= movemx; move++) {
       ...
       */
      forces(npart, x, f, side, rcoff, tmpf, nb_threads);
